...
def list_leagues(SportTypeId, label):
    SportTypeId = int(SportTypeId)
    SportLeagues = get_SportLeagues(SportTypeId = SportTypeId)
    for SportLeague in SportLeagues:
        list_item = xbmcgui.ListItem(label = SportLeague['league'])
        url = get_url(action='list_league', SportTypeId = SportTypeId, SportLeagueId = SportLeague['id'], label = label + ' \ ' + SportLeague['league'])
        xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
    xbmcplugin.endOfDirectory(_handle)

def list_submenu(SportTypeId, label):
    xbmcplugin.setPluginCategory(_handle, label)
    SportTypeId = int(SportTypeId)
    ...
    if SportTypeId >= 0:
        ...
        url = get_url(action='list_leagues', SportTypeId = SportTypeId, label = label + ' \ ' + 'Soutěže')  
        xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
        ...
        for SportType in SportTypes:
            if SportType['other'] == True:
                ...
                url = get_url(action='list_submenu', SportTypeId = SportType['id'], label = label + ' \ ' + SportType['sport'])
                xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
    xbmcplugin.endOfDirectory(_handle)

def list_today(label):
    xbmcplugin.setPluginCategory(_handle, label)
    post = {'Lang' : 'cz'}
...
def list_settings(label):
    xbmcplugin.setPluginCategory(_handle, label)
    list_item = xbmcgui.ListItem(label = 'Výběr sportů a soutěží')
        ...
        if 'SportTypeIds' not in blacklist or SportType['id'] not in blacklist['SportTypeIds']:
            if SportType['id'] >= 0:
                if SportType['id'] in mainlist:
                    ...
                    list_item.addContextMenuItems([('Zakázat', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_blacklist&toggle=1&SportTypeId=' + str(SportType['id']) + ')'),
                                                    ('Odebrat z hlavní obrazovky', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_mainlist&toggle=0&SportTypeId=' + str(SportType['id']) + ')')])                   
                else:
                    list_item = xbmcgui.ListItem(label = SportType['sport'])
                    list_item.addContextMenuItems([('Zakázat', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_blacklist&toggle=1&SportTypeId=' + str(SportType['id']) + ')'),
                                                    ('Přidat na hlavní obrazovku', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_mainlist&toggle=1&SportTypeId=' + str(SportType['id']) + ')')])                   
        else:
            if SportType['id'] in mainlist:
                list_item = xbmcgui.ListItem(label = '[B][COLOR=gray]' + SportType['sport'] + '[/COLOR][/B]')
                list_item.addContextMenuItems([('Povolit', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_blacklist&toggle=0&SportTypeId=' + str(SportType['id']) + ')'),                   
                                                ('Odebrat z hlavní obrazovky', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_mainlist&toggle=0&SportTypeId=' + str(SportType['id']) + ')')])                   
            else:
    ...
    for SportLeague in SportLeagues:
        if SportLeague['id'] > 0:
            if 'SportLeagueIds' not in blacklist or SportLeague['id'] not in blacklist['SportLeagueIds']:
                list_item = xbmcgui.ListItem(label = SportLeague['league'])
                list_item.addContextMenuItems([('Zakázat', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_blacklist&toggle=1&SportLeagueId=' + str(SportLeague['id']) + ')')])                   
            else:            
                list_item = xbmcgui.ListItem(label = '[COLOR=gray]' + SportLeague['league'] + '[/COLOR]')
                list_item.addContextMenuItems([('Povolit', 'RunPlugin(plugin://plugin.video.tvcom.cz?action=change_blacklist&toggle=0&SportLeagueId=' + str(SportLeague['id']) + ')')])                   
    ...
    if len(mainlist) == 0:
        for SportType in SportTypes:
            if SportType['other'] == False:
                list_item = xbmcgui.ListItem(label = SportType['sport'])
                url = get_url(action='list_submenu', SportTypeId = SportType['id'], label = SportType['sport'])
                xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
    else:
        for SportType in SportTypes:
            if SportType['id'] in mainlist:
...
def router(paramstring):
    ...
    if params:
        if params['action'] == 'list_today':
            list_today(params['label'])
        elif params['action'] == 'list_submenu':
            list_submenu(params['SportTypeId'], params['label'])
        elif params['action'] == 'list_leagues':
            list_leagues(params['SportTypeId'], params['label'])
